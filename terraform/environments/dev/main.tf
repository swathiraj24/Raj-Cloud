...
module "network" {
  ...
  spoke_subnet_prefixes = {
    "spoke-vnet-apps" = "10.1.1.0/24"
    "spoke-vnet-aks"  = "10.2.1.0/24"
  }
}
...
module "vpn" {
  ...
  create_local_network_gateway   = false # set to true when you have an on-prem public IP and local address prefixes
  local_network_gateway_ip_address = var.local_network_gateway_ip_address
  local_address_space            = var.local_address_space
  shared_key                     = var.vpn_shared_key
  create_connection              = false # set to true for real site-to-site connectivity
}
*/
